...
def test_easy_file_task_prefers_free_adequate_worker() -> None:
    codex = WorkerProfile(
        ...
        harness="codex",
        model="default",
        capabilities={"repo_navigation": 0.95, "simple_tasks": 0.95},
        reliability=0.82,
        speed=0.45,
    ...
    flash = WorkerProfile(
        ...
        harness="gemini",
        model="flash",
        capabilities={"repo_navigation": 0.85, "simple_tasks": 0.9},
        reliability=0.85,
        speed=0.95,
    ...
    scorer = WorkerScorer()
    scores = [
        scorer.score(p, make_analysis({"repo_navigation": 0.9, "simple_tasks": 0.8}))
        for p in [codex, flash]
    ]
