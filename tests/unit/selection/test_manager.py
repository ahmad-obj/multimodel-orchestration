...
def w(worker_id, can_manage, score):
    p = WorkerProfile(
        ...
        harness="x",
        model="m",
        capabilities={"reasoning": score, "coding": score},
        reliability=0.9,
        speed=0.7,
...
def analysis():
    return TaskAnalysis(
        ...
        confidence=0.9,
        capability_weights={"reasoning": 0.9, "coding": 0.8},
...
def test_non_manager_worker_is_never_ranked() -> None:
    selection = ManagerSelector().rank(
        analysis(), [w("strong", False, 0.99), w("manager", True, 0.75)]
    )
    assert [r.worker.profile.id for r in selection.ranked] == ["manager"]


def test_close_manager_scores_need_tiebreak() -> None:
    selection = ManagerSelector().rank(analysis(), [w("a", True, 0.85), w("b", True, 0.84)])
