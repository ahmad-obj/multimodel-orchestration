...
def d(worker_id):
    p=WorkerProfile(id=worker_id,harness="x",model="m",capabilities={"reasoning":.8,"simple_tasks":.8},reliability=.8,speed=.8,cost_class=CostClass.FREE,parallel_capacity=1,tools={"filesystem"},can_manage=True,is_paid=False)
...
class FakeExecutor:
    async def __call__(self,*args,**kwargs):
        ...
        return WorkerResult(execution_id="e",worker_id=worker.profile.id,task_id="router",status=ExecutionStatus.SUCCEEDED,summary="ok",structured_output=decision.model_dump()),decision


async def test_router_cannot_choose_non_candidate() -> None:
    analysis=TaskAnalysis(task_type="x",complexity=TaskComplexity.LOW,risk=TaskRisk.LOW,confidence=.9,capability_weights={"reasoning":.8},required_tools={"filesystem"},constraints=[],expected_outputs=["x"],parallelizable_hint=False)
    candidates=[RankedManager(worker=d("a"),score=0.8,reason="x"),RankedManager(worker=d("b"),score=0.79,reason="y")]
    registry=type("R",(),{"available":lambda self:[d("router")],"adapters":{"x":object()}})()
