...
class FakeAnalyzer:
    async def analyze(self, user_request, repository_summary):
        return TaskAnalysis(task_type="repository_inspection",complexity=TaskComplexity.LOW,risk=TaskRisk.LOW,confidence=.9,capability_weights={"repo_navigation":.9,"simple_tasks":.8},required_tools={"filesystem"},constraints=[],expected_outputs=["file list"],parallelizable_hint=False)


class FakeAdapter:
    async def execute(self, worker, request):
        return WorkerResult(execution_id="e",worker_id=worker.profile.id,task_id=request.task_id,status=ExecutionStatus.SUCCEEDED,summary="done")


def make(worker_id,cost,nav,simple,reliability,speed):
    p=WorkerProfile(id=worker_id,harness=worker_id.split('/')[0],model="m",capabilities={"repo_navigation":nav,"simple_tasks":simple},reliability=reliability,speed=speed,cost_class=cost,parallel_capacity=1,tools={"filesystem"},is_paid=False)
...
async def test_easy_task_selects_free_worker(tmp_path) -> None:
    workers=[make("codex/default",CostClass.INCLUDED,.95,.95,.82,.45),make("gemini/flash",CostClass.FREE,.85,.9,.85,.95)]
