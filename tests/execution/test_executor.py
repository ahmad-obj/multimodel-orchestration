...
class Registry:
    def __init__(self, capacity):
        ...
        self.adapters = {"x": FakeAdapter(0.20)}
...
async def test_executor_runs_independent_tasks_concurrently(tmp_path):
    ...
    results = await ex.execute_many([assignment("T1", tmp_path), assignment("T2", tmp_path)])
...
async def test_executor_respects_worker_parallel_capacity(tmp_path):
    ...
    started = time.perf_counter()
    await ex.execute_many([assignment("T1", tmp_path), assignment("T2", tmp_path)])
    elapsed = time.perf_counter() - started
    assert elapsed >= 0.38
