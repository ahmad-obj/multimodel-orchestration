...
def test_failure_classifier_typed_cases():
    ...
    assert classifier.classify(result=result(ExecutionStatus.TIMED_OUT)) is FailureClass.TIMEOUT
    try:
        WorkerResult.model_validate({})
    except ValidationError as exc:
        assert classifier.classify(error=exc) is FailureClass.INVALID_STRUCTURED_RESPONSE

    verification = VerificationResult(passed=False, checks=[], summary="failed")
    assert (
        classifier.classify(result=result(ExecutionStatus.SUCCEEDED), verification=verification)
        is FailureClass.IMPLEMENTATION_FAILURE
    )
    assert (
        classifier.classify(result=result(errors=["cannot solve: missing required capability"]))
