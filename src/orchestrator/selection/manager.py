...
class ManagerSelector:
    ...
    def rank(self, analysis: TaskAnalysis, workers: list[WorkerDescriptor]) -> ManagerSelection:
        filtered = eligible_workers(
            workers, analysis, self.cost_policy, for_manager=True, requires_write=False
        )
        ...
        for worker in filtered.eligible:
            ...
            if not score.adequate:
                continue
            reliability_bonus = (
                min(worker.profile.reliability * 1.10, 1.0) - worker.profile.reliability
            )
            ...
            ranked.append(
                RankedManager(
                    ...
                    score=round(total, 6),
                    reason=(
                        f"capability={score.capability_match:.3f}, "
                        f"reliability={worker.profile.reliability:.3f}, "
                        f"speed={worker.profile.speed:.3f}, "
                        f"cost={worker.profile.cost_class.value}"
                    ),
                )
            )
        ranked.sort(key=lambda item: item.score, reverse=True)
        needs_tiebreak = (
            len(ranked) >= 2 and ranked[0].score - ranked[1].score < self.CLEAR_WINNER_GAP
        )
        return ManagerSelection(
            ranked=ranked[:3], rejected=filtered.rejected, needs_tiebreak=needs_tiebreak
        )
