...
class OpenCodeAdapter(WorkerAdapter):
    ...
    def __init__(self, executable: Path | None = None, model: str = "auto", runner: ProcessRunner | None = None) -> None:
        self.executable = executable or (Path(found) if (found := shutil.which("opencode")) else None)
        self.model = model
    ...
    def build_command(self, workspace: Path, prompt: str, *, model: str | None = None) -> list[str]:
        ...
        return [
            str(self.executable), "run", prompt, "--model", model or self.model,
    ...
    async def discover(self) -> list[WorkerDescriptor]:
        ...
        profile = WorkerProfile(
            id=f"opencode/{self.model}", harness="opencode", model=self.model,
            capabilities={}, reliability=0.5, speed=0.6, cost_class=CostClass.FREE,
            parallel_capacity=1, tools={"filesystem", "shell", "git"},
            can_manage=True, can_modify_repo=True, is_paid=False,
        )
        descriptor = WorkerDescriptor(profile=profile, executable_path=self.executable, status=WorkerStatus.AVAILABLE)
    ...
    async def health_check(self, worker: WorkerDescriptor) -> WorkerDescriptor:
        ...
        outcome = await self.runner.run([str(worker.executable_path), "run", "--help"], cwd=Path.cwd(), timeout_seconds=10)
        ...
        if outcome.returncode != 0 or any(flag not in text for flag in required):
            return worker.model_copy(update={"status": WorkerStatus.UNAVAILABLE, "health_reason": "required run flags unsupported"})
    ...
    async def execute(self, worker: WorkerDescriptor, request: WorkerRequest) -> WorkerResult:
        ...
        outcome = await self.runner.run(
            cmd, cwd=workspace, timeout_seconds=request.timeout_seconds,
            env=env, execution_id=execution_id,
        )
        if outcome.timed_out:
            return WorkerResult(execution_id=execution_id, worker_id=worker.profile.id, task_id=request.task_id, status=ExecutionStatus.TIMED_OUT, summary="OpenCode timed out", errors=[outcome.stderr], duration_seconds=outcome.duration_seconds)
        if outcome.returncode != 0:
            return WorkerResult(execution_id=execution_id, worker_id=worker.profile.id, task_id=request.task_id, status=ExecutionStatus.FAILED, summary="OpenCode failed", errors=[outcome.stderr], duration_seconds=outcome.duration_seconds)
        final_text = ""
        ...
        try:
            parsed = json.loads(final_text) if request.expected_output_schema is not None else {"summary": final_text}
        except Exception as exc:
            return WorkerResult(execution_id=execution_id, worker_id=worker.profile.id, task_id=request.task_id, status=ExecutionStatus.FAILED, summary="OpenCode returned invalid structured output", errors=[str(exc)], duration_seconds=outcome.duration_seconds, usage={"unknown_events": unknown})
        return WorkerResult(
            execution_id=execution_id, worker_id=worker.profile.id, task_id=request.task_id,
            status=ExecutionStatus.SUCCEEDED, summary=str(parsed.get("summary", "")),
            structured_output=dict(parsed.get("structured_output", {})),
            confidence=float(parsed.get("confidence", 0.0)),
            usage={"unknown_events": unknown}, duration_seconds=outcome.duration_seconds,
        )
