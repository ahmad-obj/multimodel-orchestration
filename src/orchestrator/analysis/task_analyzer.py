...
class TaskAnalyzer:
    ...
    def _select_worker(self):
        ...
        candidates.sort(key=lambda w: (order[w.profile.cost_class], -w.profile.speed, -w.profile.reliability))
        return candidates[0]

    async def analyze(self, user_request: str, repository_summary: RepositorySummary) -> TaskAnalysis:
        worker = self._select_worker()
        adapter = self.registry.adapters[worker.profile.harness]
        prompt = (
            "Analyze this coding task for orchestration. Return only the requested structured JSON.\n"
            f"User request: {user_request}\n"
            f"Repository summary: {json.dumps(repository_summary.model_dump(mode='json'), sort_keys=True)}\n"
            f"Capability dimensions: {sorted(BASE_CAPABILITY_DIMENSIONS)}\n"
            "Use normalized capability weights from 0 to 1. Identify required tools, risk, complexity, "
            "context requirements, expected outputs, and whether independent parallel work is plausible."
        )
