    ...
    async def create_plan(
        ...
        *,
        ...
        prompt = (
            ...
            "Use outcome-oriented subtasks; explicit dependency IDs; read_only flags; required tools/capabilities/context; "
            "write_paths for modifying tasks that may run in parallel; real parallel groups only; no worker IDs and no spawning. "
            "Report confidence and human_question only when material information is missing.\n"
            f"Objective: {objective}\n"
            f"Analysis: {json.dumps(analysis.model_dump(mode='json'), sort_keys=True)}\n"
            f"Repository summary: {json.dumps(repository_summary.model_dump(mode='json') if hasattr(repository_summary,'model_dump') else repository_summary, default=str, sort_keys=True)}"
        )
